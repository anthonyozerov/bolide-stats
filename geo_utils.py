from math import sqrt, degrees, radians, acos, asin, cos, sin
...
def get_areas(polygons, crs):

    areas = []
    for poly in polygons:
        centroid = poly.centroid
        centroid = GeoDataFrame(geometry=[centroid], crs=crs).to_crs('epsg:4326').geometry[0]
        lon = centroid.x
        lat = centroid.y
